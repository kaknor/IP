...
def create_line(f, d_fasta, i, tmd):
    ...
    f.write(get_prot_seq(d_fasta, i[0], (tmd[2], tmd[3])) + '\t')
    f.write(str(tmd[0]) + ' - ' + str(tmd[1]) + '\t')
    f.write(get_prot_seq(d_fasta, i[0], (tmd[0], tmd[1])) + '\t')

def create_last_ntmd(f, d_fasta, i, lbound):
    l = general_file_data[i[0]][0]
    f.write(i[0] + '\t')
    f.write(i[1].strip('\n') + '\t')
    f.write('N/A\t')
    f.write('N/A\t')
    seq = get_prot_seq(d_fasta, i[0], (lbound, l))
